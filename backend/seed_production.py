"""
Run this script to seed student applications into the PRODUCTION database.
Usage: python seed_production.py
Make sure DATABASE_URL is set in your .env file pointing to the production PostgreSQL DB.
"""
import os
import random
...
# Load .env first so DATABASE_URL is set before app is created
load_dotenv()
    ...
    with flask_app.app_context():
        ...
        for s in students:
            ...
            app = StudentApplication(
                ...
                caste=s["caste"],
                religion=s["rel"],
                fee_concession="Yes" if get_category(s["caste"]) in ["SC", "ST"] else "No",
                nationality="Indian",
        ...
        db.session.commit()
        print(f"\n✅ Seeded {count} students! ({skipped} already existed)")
